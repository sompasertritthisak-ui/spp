...
import type { Billboard, BlogPost, Bundle, Category, DesignTemplate, Faq, FeatureFlags, PortfolioProject, Product, Service, SiteContent, SiteSettings, Solution, Testimonial } from "@/content/types";
...
async function fromDatabase(): Promise<SiteContent> {
  const [settingsRows, flagRows, cats, prods, rels, svcs, sols, bnds, bitems, bbs, port, faqRows, tms, blog, tpls] = await Promise.all([
  ...
  const portfolio: PortfolioProject[] = port.map((p) => ({
    ...
    palette: ((p.palette as string[]) ?? ["#ffd60a", "#17171a", "#f5f5f2"]).slice(0, 3) as [string, string, string], study: (p.study as PortfolioProject["study"]) ?? [], impact: (p.impact as PortfolioProject["impact"]) ?? [],
  }));
  ...
  const posts: BlogPost[] = blog.map((p) => ({ slug: s(p.slug), title: s(p.title), excerpt: s(p.excerpt), body: s(p.body), tag: s(p.tag), readMins: n(p.read_mins) ?? 3, date: s(p.published_at).slice(0, 10) }));
