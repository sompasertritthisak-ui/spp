...
export function FaqList({ faqs }: { faqs: Faq[] }) {
  return (
    <div className="border-b border-paper-ink">
      {faqs.map((f, i) => (
        <details key={f.q} className="group border-t border-paper-ink first:border-t-2" open={i === 0}>
          <summary className="flex min-h-11 list-none items-baseline gap-4 py-6 [&::-webkit-details-marker]:hidden">
            ...
            <span aria-hidden className="relative h-4 w-4 flex-none self-center">
              <span className="absolute inset-x-0 top-1/2 h-px bg-paper-ink" />
              <span className="absolute inset-y-0 left-1/2 w-px bg-paper-ink transition-transform duration-300 ease-[var(--ease-press)] group-open:scale-y-0" />
            </span>
          </summary>
