...
export function GoalTeaser({ solutions }: { solutions: Solution[] }) {
  ...
  return (
    <section aria-labelledby="goals-title" className="border-y border-ink-700 bg-ink-900">
      <div className="shell grid gap-x-16 gap-y-12 py-20 lg:grid-cols-12 lg:py-32">
        ...
        <ol className="border-b border-ink-700 lg:col-span-7">
          {list.map((s, i) => (
            <Reveal as="li" key={s.slug} i={Math.min(i, 5)}>
              <Link href={`/solutions/?goal=${s.slug}`} className="group/btn grid grid-cols-[2.5rem_1fr_auto] items-baseline gap-x-4 border-t border-ink-700 py-6 transition-colors duration-200 hover:bg-ink-850 sm:grid-cols-[3.5rem_1fr_auto] lg:py-7">
                <span className="t-data text-xs text-fog-500 transition-colors group-hover/btn:text-yellow">{String(i + 1).padStart(2, "0")}</span>
