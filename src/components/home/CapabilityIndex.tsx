...
export function CapabilityIndex({ categories, products }: { categories: Category[]; products: Product[] }) {
  ...
  return (
    <section aria-labelledby="capabilities-title" className="border-y border-ink-700 bg-ink-900">
      <div className="shell py-20 lg:py-32">
        ...
        <ol className="border-b border-ink-600">
          {list.map((c) => {
            const count = products.filter((p) => p.category === c.slug).length;
            return (
              <li key={c.slug}>
                <Link href={`/products/?category=${c.slug}`} className="group/btn relative grid grid-cols-[3rem_1fr_auto] items-baseline gap-x-4 border-t border-ink-600 py-6 transition-colors duration-200 hover:bg-ink-850 lg:grid-cols-[6rem_minmax(0,5fr)_minmax(0,6fr)_5rem_auto] lg:gap-x-8 lg:py-8">
