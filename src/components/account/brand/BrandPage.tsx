...
const schema = z.object({
  ...
  colours: z.array(z.object({ name: z.string().trim().max(40), hex: z.string().regex(HEX, "Each colour needs a 6-digit hex value such as #FFD60A.") })).max(MAX_SWATCHES),
...
export function BrandPage() {
  ...
  const [colours, setColours] = useState<Swatch[]>([]);
  ...
  useEffect(() => {
    if (q.loading || dirty) return;
    setName(q.data?.brand_name ?? "");
    setColours(readSwatches(q.data?.colours));
    setFonts(q.data?.fonts ?? []);
    setGuidelines(q.data?.guidelines ?? "");
  ...
  return (
    <>
      ...
      <Block title="Brand profile">
        {q.loading && !q.data && !dirty ? <RowsSkeleton rows={4} /> : (
          <form onSubmit={save} noValidate className="flex max-w-3xl flex-col gap-8">
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <p className="text-sm text-fog-500" aria-live="polite">{dirty ? "Unsaved changes." : q.data ? "All changes saved." : "Nothing saved yet."}</p>
            </div>
          </form>
        )}
      </Block>
