...
export function ProjectDetail({ id }: { id: string }) {
  ...
  if (!p || !q.data) return <><PortalHeader title="Project not found" back={back} /><PortalEmpty title="We could not find that project." body="It may belong to a different account, or the link may be incomplete." action={<Button href="/account/projects/" variant="outline">All projects</Button>} /></>;

  const done = q.data.milestones.filter((m) => m.done_at).length;
  const next = q.data.milestones.findIndex((m) => !m.done_at); // the milestone in play
  return (
    <>
      <PortalHeader title={p.name || "Untitled project"} back={back} sub={<span className="flex flex-wrap items-center gap-3"><StatusPill status={p.stage} /><span className="t-data text-gold">{p.ref}</span>{p.due_on && <span>Due {formatDate(p.due_on)}</span>}</span>} />

      <section aria-label="Project stage" className="glow-brand relative isolate mb-10 border border-gold/25 bg-ink-900 p-5">
      ...
      <Block title={`Milestones${q.data.milestones.length ? ` · ${done} of ${q.data.milestones.length} done` : ""}`}>
        {q.data.milestones.length === 0 ? <p className="border border-dashed border-gold/40 p-5 text-fog-400">SPP has not set milestones for this project yet. They appear here as soon as the plan is agreed.</p> : (
          <ol className="border-t border-gold/25">
            {q.data.milestones.map((m, i) => (
              <li key={m.id} className="flex items-center gap-4 border-b border-ink-700 py-3">
                <span aria-hidden className={`flex h-6 w-6 flex-none items-center justify-center border ${m.done_at ? "border-ok bg-ok text-ink-950" : i === next ? "border-gold text-gold" : "border-ink-500 text-fog-500"}`}>{m.done_at ? <Check className="h-3.5 w-3.5" strokeWidth={2} /> : <span className="t-data text-[0.625rem]">{i + 1}</span>}</span>
