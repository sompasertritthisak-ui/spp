    ...
    <>
      ...
      {q.loading && !q.data ? <RowsSkeleton rows={5} /> : notes.length === 0 ? (
        ...
        <ul className="border-t border-gold/25" aria-live="polite">
          {notes.map((n) => (
            <li key={n.id} className={`flex items-start gap-4 border-b border-l-2 border-ink-700 py-4 pl-3 ${n.read_at ? "border-l-transparent" : "border-l-gold"}`}>
              <span aria-hidden className={`mt-2 h-2 w-2 flex-none ${n.read_at ? "bg-ink-600" : "bg-gold"}`} />
