...
export function QuoteSummary({ lines, bySlug, band, live, neededBy, bundleNote }: { lines: Line[]; bySlug: Map<string, ProductLite>; band: Band; live: boolean; neededBy: string; bundleNote?: string }) {
  return (
    <div>
      <p className="on-gold t-label flex items-center gap-3 px-7 py-3.5"><span aria-hidden className="reg" />Your request</p>
      <div className="p-7">
        {lines.length === 0 ? <p className="text-fog-400">Nothing added yet.</p> : (
