...
export function Section({ children, tone = "ink", className = "", id }: { children: ReactNode; tone?: "ink" | "paper" | "raised"; className?: string; id?: string }) {
  const tones = { ink: "bg-ink-950", raised: "bg-ink-900 border-y border-ink-700", paper: "on-paper" } as const;
  return (
...
export function SectionHead({ plate, eyebrow, title, lede, tone = "ink", action }: { plate?: string; eyebrow: string; title: ReactNode; lede?: ReactNode; tone?: "ink" | "paper"; action?: ReactNode }) {
  return (
    <header className="mb-12 grid gap-6 lg:mb-16 lg:grid-cols-[1fr_auto] lg:items-end">
      <div className="max-w-4xl">
        ...
        <h2 className={`t-display ${tone === "paper" ? "text-paper-ink" : "text-fog-50"}`}>{title}</h2>
        {lede && <p className={`mt-6 max-w-2xl text-lg leading-relaxed ${tone === "paper" ? "text-paper-mute" : "text-fog-300"}`}>{lede}</p>}
      </div>
      {action}
    </header>
  );
