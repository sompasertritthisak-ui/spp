...
export function LocatorMap({ site, others }: { site: Site; others: Site[] }) {
  const r = 9;
  return (
    <figure className="relative border border-ink-700 bg-ink-950">
      <svg viewBox={`0 0 ${VIEWBOX.w} ${VIEWBOX.h}`} role="img" aria-label={`Locator map: ${site.name} in ${site.province}, Laos`} className="block h-auto w-full">
        {PROVINCES.map((p) => <path key={p.id} d={p.d} strokeWidth={1} vectorEffect="non-scaling-stroke" className={p.id === site.provinceId ? "fill-ink-700 stroke-ink-500" : "fill-ink-850 stroke-ink-700"} />)}
        <path d={OUTLINE} fill="none" strokeWidth={1.25} vectorEffect="non-scaling-stroke" className="stroke-fog-500" />
      ...
      <figcaption className="t-label flex flex-wrap justify-between gap-2 border-t border-ink-700 px-4 py-3 text-[0.625rem] text-fog-400"><span>{site.province}</span><span className="t-data normal-case tracking-normal">{dms(site.lat, site.lng)}</span></figcaption>
    </figure>
  );
}
