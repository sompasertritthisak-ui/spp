 ...
 *   available  solid gold disc       reserved     half-filled sky ring
 *   maintenance  open orange diamond  unavailable  muted cross
 */
export function StatusGlyph({ status, size = 16, className }: { status: BillboardStatus; size?: number; className?: string }) {
  ...
  switch (status) {
    ...
    case "reserved":
      return <svg {...common}><circle cx="8" cy="8" r="5.5" className="fill-ink-950 stroke-sky" strokeWidth="1.5" /><path d="M8 2.5a5.5 5.5 0 0 0 0 11z" className="fill-sky" /></svg>;
    case "maintenance":
...
const tone: Record<BillboardStatus, string> = { available: "text-yellow", reserved: "text-sky", maintenance: "text-warn", unavailable: "text-fog-400" };
