...
export function SiteCard({ site, showPrices, onClose }: { site: Site; showPrices: boolean; onClose: () => void }) {
  ...
  return (
    <article aria-labelledby={`card-${site.code}`} className="border border-ink-600 bg-ink-900/95 backdrop-blur-sm [animation:register_.3s_var(--ease-press)_both]">
      <header className="flex items-start justify-between gap-3 border-b border-ink-700 p-5 pb-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-b border-ink-700 px-5 py-3">
      ...
      <dl className="grid grid-cols-[auto_1fr] gap-x-5 gap-y-2 px-5 py-4 text-sm">
        ...
        <div className="contents"><dt className="t-label pt-0.5 text-[0.625rem] text-fog-500">Guide</dt><dd className="t-data text-fog-50">{price ?? "Price on request"}</dd></div>
      </dl>
      ...
      <footer className="flex flex-wrap gap-2 border-t border-ink-700 p-4">
