...
export const MapBase = memo(function MapBase({ active, focused, onHover, onPick }: { active: ReadonlySet<string>; focused: string | null; onHover: (id: string | null) => void; onPick: (id: string) => void }) {
  return (
    <g>
      {PROVINCES.map((p) => {
        ...
        return (
          <path
            ...
            className={clsx("transition-[fill] duration-200 ease-[var(--ease-press)]", on ? "cursor-pointer fill-ink-800 stroke-gold/45 hover:fill-ink-700" : "fill-ink-900 stroke-gold/20")}
      ...
      {focused && PROVINCES.filter((p) => p.id === focused).map((p) => <path key={p.id} d={p.d} fill="none" vectorEffect="non-scaling-stroke" strokeWidth={1.5} className="pointer-events-none stroke-gold" />)}
      <path d={OUTLINE} fill="none" vectorEffect="non-scaling-stroke" strokeWidth={1.25} strokeLinejoin="round" className="pointer-events-none stroke-gold/70" />
    </g>
  );
});
