...
export function MarkG({ stroke = "var(--color-fog-50)", dot = "var(--color-gold)", ...rest }: { stroke?: string; dot?: string } & SVGProps<SVGGElement>) {
  return (
    <g {...rest}>
...
export function Construction() {
  ...
  return (
    <svg viewBox="-30 -40 276 190" role="img" aria-label="Construction of the SPP wordmark from a single circle of radius 20" className="block h-auto w-full">
      ...
      <g stroke="var(--color-fog-500)" strokeWidth=".5" strokeDasharray="2 2"><path d="M-30 0H246M-30 100H246" /></g>
      <MarkG stroke="var(--color-fog-50)" opacity=".92" />
      ...
      <g stroke="var(--color-gold)" strokeWidth=".6" fill="none">
      ...
      <g fontFamily={MONO} fontSize="5.2" letterSpacing=".5" fill="var(--color-fog-300)">
        ...
        <text x="196" y="22" fill="var(--color-gold)">registration dot · r 5.5</text>
        <text x="-20" y="-24" fill="var(--color-fog-500)">grid: 20 units</text>
      </g>
    </svg>
  );
}
...
export function LogoVersions() {
  const versions: Version[] = [
    { file: "spp-wordmark-ink.svg", name: "Wordmark — on indigo", use: "The primary version: cool white strokes, gold dot. Use on midnight indigo, SPP Navy and any dark ground.", ground: "bg-ink-950", render: <Logo className="h-14 w-auto sm:h-20" /> },
    { file: "spp-wordmark-paper.svg", name: "Wordmark — on white", use: "Indigo strokes, gold dot. For white, light paper and light fabric.", ground: "bg-paper", render: <Logo tone="paper" className="h-14 w-auto sm:h-20" /> },
    ...
    { file: "spp-icon.svg", name: "Icon — gold plate", use: "Favicons, app icons, avatars, stickers. Only when the square is under 64 px or the wordmark will not fit.", ground: "bg-ink-800", render: <LogoPlate className="h-24 w-24 sm:h-28 sm:w-28" /> },
  ];
  return (
...
export function ClearSpace() {
  return (
    <svg viewBox="-70 -70 356 240" role="img" aria-label="Clear space diagram: keep a margin of two stroke-widths on every side of the wordmark" className="block h-auto w-full">
      <rect x="-40" y="-40" width="296" height="180" fill="var(--color-gold)" fillOpacity=".07" stroke="var(--color-gold)" strokeWidth=".8" strokeDasharray="4 3" />
      <rect x="0" y="0" width="216" height="100" fill="none" stroke="var(--color-ink-500)" strokeWidth=".6" />
      <MarkG />
      ...
      <g stroke="var(--color-fog-300)" strokeWidth=".8"><path d="M74 -52H94M74 -56V-48M94 -56V-48" /></g>
      <text x="100" y="-49" fontFamily={MONO} fontSize="8" fill="var(--color-fog-300)">x = stroke width</text>
    </svg>
  );
}
...
const MISUSE: { label: string; art: ReactNode; ground?: string }[] = [
  ...
  { label: "Don’t recolour the strokes — only cool white or indigo.", art: <MarkG stroke="var(--color-magenta)" transform="translate(30 30) scale(.9)" /> },
  { label: "Don’t move, resize or remove the dot.", art: <g transform="translate(30 30) scale(.9)"><MarkG dot="none" /><circle cx="110" cy="76" r="11" fill="var(--color-gold)" /></g> },
  { label: "Don’t outline it.", art: <g transform="translate(30 30) scale(.9)"><MarkG dot="none" /><g fill="none" stroke="var(--color-ink-850)" strokeWidth={16.5}><path d={LOGO_PATHS.s} /><path d={LOGO_PATHS.p1} /><path d={LOGO_PATHS.p2} /></g><circle {...LOGO_DOT} fill="none" stroke="var(--color-fog-50)" strokeWidth="1.5" /></g> },
  { label: "Don’t place it on a ground without contrast.", ground: "var(--color-gold-deep)", art: <MarkG stroke="var(--color-gold)" dot="var(--color-fog-50)" transform="translate(30 30) scale(.9)" /> },
  { label: "Don’t add shadows, glows or gradients.", art: <g transform="translate(30 30) scale(.9)"><MarkG stroke="var(--color-violet)" dot="var(--color-violet)" transform="translate(7 7)" opacity=".8" /><MarkG /></g> },
  { label: "Don’t crowd it — respect the clear space.", art: <g><MarkG transform="translate(30 30) scale(.9)" /><text x="128" y="146" textAnchor="middle" fontFamily="var(--font-bricolage), sans-serif" fontWeight="800" fontSize="22" fill="var(--color-fog-50)">MEGA SALE!!! 50% OFF</text><rect x="4" y="4" width="248" height="22" fill="var(--color-magenta)" /></g> },
];
