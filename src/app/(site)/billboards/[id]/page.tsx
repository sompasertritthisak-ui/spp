...
export default async function BillboardPage({ params }: Props) {
  ...
  const specs: [string, React.ReactNode][] = [
    ...
    ["Faces", <span key="v" className="text-gold">{b.faces === 2 ? "2 — double-sided" : "1 — single-sided"}</span>],
    ["Facing", b.facing],
  ...
  return (
    <>
      ...
      <section className="grain glow-brand relative isolate overflow-hidden border-b border-gold/30 pt-[calc(var(--nav-h)+3rem)] lg:pt-[calc(var(--nav-h)+5rem)]">
        ...
        <div className="shell pb-12 lg:pb-16">
          <nav aria-label="Breadcrumb" className="t-label mb-8 flex flex-wrap items-center gap-2 text-[0.625rem] text-fog-500">
            <Link href="/" className="hover:text-gold">Home</Link><span aria-hidden>/</span>
            <Link href="/billboards/" className="hover:text-gold">Billboards</Link><span aria-hidden>/</span>
          ...
          <div className="grid gap-10 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            <dl className="grid grid-cols-2 gap-px border border-gold/40 bg-gold/40 lg:w-[24rem]">
        ...
        <div className="shell grid gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,24rem)] lg:gap-20">
          <div>
            ...
            <dl className="border-t border-gold/40">
              {specs.map(([k, v]) => <div key={k} className="grid gap-1 border-b border-gold/20 py-4 sm:grid-cols-[11rem_1fr] sm:gap-6"><dt className="t-label pt-1 text-[0.625rem] text-fog-500">{k}</dt><dd className="text-fog-100">{v}</dd></div>)}
            </dl>
          </div>
      ...
      {nearby.length > 0 && (
        <section aria-labelledby="near-h" className="border-t border-gold/30 bg-ink-900 py-16 lg:py-24">
          <div className="shell">
            ...
            <ul className="border-t border-gold/40">
              {nearby.map(({ site: s, km }) => (
                <li key={s.code} className="border-b border-gold/20">
                  <Link href={`/billboards/${s.code}/`} className="group grid min-h-16 grid-cols-[1fr_auto] items-center gap-x-6 gap-y-1 py-4 sm:grid-cols-[7rem_1fr_auto_auto]">
                    ...
                    <span className="t-data text-sm text-gold">{km < 10 ? km.toFixed(1) : Math.round(km)} km</span>
                  </Link>
                </li>
              ))}
            </ul>
